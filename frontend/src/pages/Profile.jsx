...
import { commentQuote, deleteQuote, dislikeQuote, fetchQuotes, likeQuote, updateQuote } from "../redux/quotes/quoteSlice";
...
// const [newQuote, setNewQuote] = useState("");
  ...
  const postCount = quotes.filter(post => post.author._id === user._id).length;
  ...
  return (
    <div >
      ...
      <h3 className="text-xl font-bold mb-4 mt-8">Quotes by {user.name}</h3>
      <ul>
      {quotes.length > 0 ? (
            quotes
              .filter((quote) => quote.author._id === user._id)
              .map((quote, index) => <li key={index}>
                <div className="border px-4 py-2 my-2 w-full  rounded-lg">
